...
namespace DTLS.Types
{
    internal class Program
    {
        ...
        public static List<uint> followedOffs = new List<uint>();
        ...
        private static void Unpack_Resource(string resourceStr)
        {
            ...
            foreach (ResourceEntryObject rsobj in rfFile.ResourceEntries)
            {
                ...
                if (!path.EndsWith("/"))
                {
                    ...
                    if (!followedOffs.Contains(lsentry.DTOffset + rsobj.OffInPack))
                    {
                        if (rsobj.CmpSize > 0)
                            fileData = GetFileDataDecompressed(lsentry.DTOffset + rsobj.OffInPack, rsobj.CmpSize,
                                lsentry.DTIndex);

                        totalsize += rsobj.CmpSize;
                        followedOffs.Add(lsentry.DTOffset + rsobj.OffInPack);
                        ...
                        var folder = Path.GetDirectoryName(outfn);
        ...
        {
            Unpack_Resource("resource");
            Unpack_Resource("resource(us_en)");
            Unpack_Resource("resource(us_fr)");
            Unpack_Resource("resource(us_sp)");
        }
        ...
        private static unsafe void PatchArchive(string resourceString, string patchFolder)
        {
            ...
            lsFile.Entries[calc_crc(resourceString)].Size = full.Length;
        ...
        private static unsafe void RebuildArchive(string resourceString, string patchFolder)
        {
            LSEntryObject _resource = lsFile.Entries[calc_crc(resourceString)];
            byte[] resource = GetFileDataDecompressed(_resource.DTOffset + (uint)_resource.PaddingLength,
                _resource.Size,
            ...
            foreach (ResourceEntryObject rsobj in rfFile.ResourceEntries.Where(rsobj => rsobj != null))
            {
                ...
                if (!path.EndsWith("/"))
                    if (File.Exists($"{patchFolder}/{path}"))
                    {
                        ...
                        // write 0xCC over unused bytes.
                        addr += compressed.Length;
                        int truncateBytes = rsobj.CmpSize - compressed.Length;
                        for (int i = 0; i < truncateBytes; i++)
                            *(byte*)(addr + i) = 0xCC;
        ...
        private static byte[] GetFileDataDecompressed(uint start, int size, int dtIndex)
        {
            ...
            int chunk_len = (int)difference + size;
